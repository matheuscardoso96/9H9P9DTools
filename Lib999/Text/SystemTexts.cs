...
namespace Lib999.Text
{
    public class SystemTexts
    {
        ...
        public List<SirSubTableV4> TablesOffsets { get; set; }

        public SystemTexts(string path)
        {
            GetData(path);
        ...
        private void GetData(string path) 
        {
            ...
            while (true)
            {
                ...
                TablesOffsets.Add(new SirSubTableV4(br));
            }
            ...
            {
                StringBlock[i].CreateAScriptV4(TablesOffsets[i].UnknownBytes);
            }
        ...
        public SystemTexts(string datPath, string txtPath)
        {
            StringBlock = new();
            TablesOffsets = new();
            GetData(datPath);
            ...
            foreach (var script in txtScript)
            {
                ...
                {
                  new Dialog999(0, 0, stgs[1].Replace(toReaplce0, string.Empty), 0)

                };

                titleAreas.Add(commandArea);

                for (int i = 2; i < stgs.Count; i++)
                {
        ...
        private void CreateDataRead(uint stringBlockSize)
        {
            ...
            Enumerable.Range(0, TablesOffsets.Count).ToList().ForEach(x => list2.AddRange( new uint[] { 0x14, 4 }));
