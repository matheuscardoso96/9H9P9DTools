...
namespace Lib999.Text
{
    public class ItemsNames
    {
        ...
        public List<SirSubTableV5> TablesOffsets { get; set; }

        public ItemsNames(string path)
        {
            GetData(path);
        ...
        private void GetData(string path)
        {
            ...
            while (true)
            {
                ...
                TablesOffsets.Add(new SirSubTableV5(br));
            }
        ...
        public ItemsNames(string datPath, string txtPath)
        {
            StringBlock = new();
            TablesOffsets = new();
            GetData(datPath);
        ...
        private void CreateDataRead(uint stringBlockSize)
        {
            ...
            {
                var count = item.SubTableArgs0.Count - 1;
                var list = new List<uint>();
                Enumerable.Range(0, count).ToList().ForEach(x => list.AddRange(new uint[] { 8, 4, 0x14  }));
                DataReadList.DecompressedValues.AddRange(list);
                DataReadList.DecompressedValues.AddRange(new uint[] { 8, 4, 0x18 });
            }
