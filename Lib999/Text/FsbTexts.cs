...
namespace Lib999.Text
{
    public class FsbTexts
    {
        ...
        private readonly List<byte> eventAreaBackup = new();
        ...
        public FsbTexts(string pathJpnScripts, string pathEngScripts, string dualPath)
        {
            ...
            {
                ProcessDualScripts(item, engScripts);
            }
        }
        ...
        public void FsbToTxt(string path, bool exportEvents)
        {
            ...
            if (exportEvents)
            {
                ...
                WriteDataReadList(destPath, path);
            }
        ...
        public void TxtToFsb(string fsbPath, string txtScriptPath)
        {
            ...
            {
                WriteInitialData(bw);
                SortAndAssignNewIds(MainStringBlock);
                WriteDialogs(bw, MainStringBlock);
                AlignStream(bw, 4, 0xAA);
                ...
                WriteOffsets(bw, MainStringBlock);
                bw.Write((uint)0);

                WriteDialogs(bw, StartStringBlock);
                AlignStream(bw, 4, 0xAA);

                FsbSubHeader.OffsetStartBlockTable = (uint)bw.BaseStream.Position;
                WriteOffsets(bw, StartStringBlock);
                bw.Write((uint)0);

                WriteDialogs(bw, StringBlock4);
                AlignStream(bw, 4, 0xAA);

                FsbSubHeader.OffsetFourthTable = (uint)bw.BaseStream.Position;
                WriteOffsets(bw, StringBlock4);
                bw.Write((uint)0);

                WriteEventDialogs(bw, EventStringsBlock);
                AlignStream(bw, 4, 0xAA);

                FsbSubHeader.EventScriptTableffset = (uint)bw.BaseStream.Position;
                WriteOffsets(bw, EventStringsBlock);
                bw.Write((uint)0);
                bw.Write((uint)0);
                ...
                UpdateEventDialogOffsets(bw, MainStringBlock);
        ...
        private void ExportAllStringBlocks(string destPath, string path)
        {
            ...
            var commandStrings = GenerateCommandStrings(MainStringBlock);
        ...
        {
            using var br1 = new BinaryReader(File.OpenRead(jpnScriptPath));
            ...
            var engScriptPath = engScripts.FirstOrDefault(x => Path.GetFileName(x) == Path.GetFileName(jpnScriptPath));
            ...
            MergeScripts(stringBlock, stringBlock2);

            File.WriteAllLines($"dual\\{Path.GetFileName(jpnScriptPath)}.txt", stringBlock2.Strings);
        }

        private void MergeScripts(SirStrings stringBlock, SirStrings stringBlock2)
        {
            ...
            {
                MergeWithMoreEngStrings(stringBlock2, justJpnWithIds);
            }
            else
            {
                MergeWithMoreJpnStrings(stringBlock2, justJpnWithIds);
            }
        }

        private void MergeWithMoreEngStrings(SirStrings stringBlock2, List<string> justJpnWithIds)
        {
            int counter = 0;
        ...
        private void MergeWithMoreJpnStrings(SirStrings stringBlock2, List<string> justJpnWithIds)
        {
            int counter = 0;
            for (int i = 0; i < stringBlock2.Strings.Count; i++)
            {
            ...
            var remaining = justJpnWithIds.Count - counter;
            stringBlock2.Strings[^1] += $"\r\n{string.Join("\r\n", justJpnWithIds.Skip(counter).Take(remaining))}";
        }

        private void WriteInitialData(BinaryWriter bw)
        {
            for (int i = 0; i < 4; i++)
            {
                bw.Write(0);
        ...
        private void SortAndAssignNewIds(SirStrings block)
        {
            ...
            for (int i = 0; i < block.Dialogs.Count; i++)
            {
                block.Dialogs[i].NewId = i;
            }
        }

        private void WriteDialogs(BinaryWriter bw, SirStrings block)
        {
            foreach (var st in block?.Dialogs)
            {
        ...
        private void AlignStream(BinaryWriter bw, int alignment, byte paddingByte)
        {
            while (bw.BaseStream.Position % alignment != 0)
            {
                bw.Write(paddingByte);
            }
        }

        private void WriteOffsets(BinaryWriter bw, SirStrings block)
        {
            foreach (var st in block?.Dialogs)
            {
                bw.Write(st.Offset);
        ...
        private void WriteEventDialogs(BinaryWriter bw, SirStrings block)
        {
            foreach (var st in block?.Dialogs)
            {
                if (!st.IsEventData)
        ...
        private void UpdateEventDialogOffsets(BinaryWriter bw, SirStrings block)
        {
            foreach (var st in block?.EventDialogs)
            {
                if (st.OffsetsWithStrings.Count > 0)
                {
                    foreach (var off in st.OffsetsWithStrings)
                    {
                        if (off.HasString)
                        {
                            var dlg = block.Dialogs.First(d => d.Id == off.Code);
        ...
        private void WriteDataReadList(string destPath, string path)
        {
        ...
        private List<string> GenerateCommandStrings(SirStrings block)
        {
            return block.Dialogs
                .Select(d => $"{d.Text} | <ID: Int {d.Id} HEX {d.Id.ToString("X")}> <OFFSET: {d.Offset.ToString("X")}>")
                .ToList();
        }
